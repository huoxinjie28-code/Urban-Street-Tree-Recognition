...
ornet = []
with open(fiLe_path + 'ornet.txt', 'r') as f:
    lines = f.readlines()
    for line in lines:
        parts = line.split('\t')
        ornet.append([parts[0], parts[1]])
...
with open(fiLe_path + 'c.txt', 'r') as f:
    ...
    for line in lines:
        parts = line.split('\t')
        index_id = [index_id for index_id, value in enumerate(ornet) if (value[0] == parts[0])]
        new_name = ornet[index_id[0][1]]
        x_mid = parts[1].split('.')[0]
        names.append([new_name.replace('"', ' ').replace('.jpg', ' ').strip(), x_mid, parts[0].replace('"', ' ').replace('.jpg', ' ').strip()])
        info = new_name.replace('"', ' ').replace('.jpg', ' ').strip().split('_')
        k = len(info)
        lat, lng, north = info[0], info[1], info[2]
        con = parts[2]
        direction = (float(x_mid)/width)*90 - north
        if direction < 0:
...
n = []
for name in names:
    index_id = [index_id for index_id, value in enumerate(sp) if (value[0] == name[2]) & (value[1] == name[1])]
